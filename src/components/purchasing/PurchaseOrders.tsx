...
interface PO {
  ...
  updated_by :number;
  created_by:number;
  items?: Array<{ item_id: number; item_name?: string; item_code: string; quantity: number; unit_price: number }>;
}
interface viewingPO {
  po_id?: number;
  vendor_id?: number;
                    ...
                    type="checkbox"
                      checked={
                       selectedPOs.length > 0 &&
                         selectedPOs.length === filteredPO.length
                           }
                           onChange={(e) => {
                           if (e.target.checked) {
                            setSelectedPOs(filteredPO.map((po) => po.po_id));
                            } else {
                               setSelectedPOs([]);
                                 }
                              }}
