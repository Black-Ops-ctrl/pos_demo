...
const BRV: React.FC = () => {
  ...
  const [editingEntry, setEditingEntry] = useState<BRV | null>(null);
  const [showScrollToTop, setShowScrollToTop] = useState(false);
  const [selectedJournalEntry, setselectedJournalEntry] = useState<number[]>([]);
  const [printingEntry, setPrintingEntry] = useState<BRV | null>(null);
  const [statusFilter, setStatusFilter] = useState<string>('ALL');
  const [entries, setEntries] = useState<BRV[]>([]);
...
const filteredJE = entries.filter((entry) => {
  ...
  const handleEditEntry = (entry: BRV) => {
    setEditingEntry(entry);
    setShowForm(true);
  };

  const handleDeleteEntry = async (entry: BRV) => {
  ...
  const handlePrintEntry = (entry: BRV) => {
...
// In the BRV component, update handleSaveEntry to properly handle async
      ...
      <Card className="bg-white/80 backdrop-blur-sm shadow-lg">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Bank Receipt Vouchers</CardTitle>
...
const BRVForm: React.FC<BRVFormProps> = ({ onClose, onSave, entry }) => {
  ...
  const [voucher_id, setVoucherId] = useState<number>(5);
  ...
  setEntryDate(formattedDate);
          ...
          if (entry.lines && Array.isArray(entry.lines) && entry.lines.length > 0) {
            ...
            if (creditLines.length > 0) {
              setEntryLines(creditLines.map((line: any) => ({
      ...
      <Card className="w-full max-w-4xl max-h-[98vh] overflow-hidden rounded-3xl border-0 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 shadow-2xl flex flex-col">
        ...
        <CardHeader className="flex flex-row items-center justify-between bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 text-white rounded-t-3xl px-4 sm:px-6 py-3 sm:py-4 flex-shrink-0">
          <CardTitle className="text-lg sm:text-xl font-bold tracking-wide text-white">
            {entry ? 'Edit BRV Entry' : 'Bank Receipt Voucher'}
          </CardTitle>
          <Button
            variant="ghost"
            size="sm"
                    ...
                    <PopoverTrigger asChild>
                      <Button
                        ...
                        className={cn(
                          ...
                          !debitAccountId && "border-red-400 focus:ring-red-400"
                        )}
                      >
                        {debitAccountId
                  ...
                  <Table>
                    <TableHeader className="sticky top-0 z-10">
                      <TableRow className="bg-gradient-to-r from-purple-500 to-indigo-500">
                        ...
                        <TableHead className="text-white py-2 text-xs sm:text-sm">Credit Amount</TableHead>
                      ...
                      {entryLines.map((line, index) => (
                        <TableRow key={index} className="hover:bg-purple-50">
                          <TableCell className="py-1 px-1 sm:px-2">
                            <Popover>
                              <PopoverTrigger asChild>
                                <Button className="w-full justify-between bg-indigo-50 border border-gray-300 text-black h-8 sm:h-9 text-xs sm:text-sm px-2">
                                  {line.account_id
                          ...
                          <TableCell className="py-1 px-1 sm:px-2">
                            <Input
                              ...
                              className="bg-indigo-50 h-8 sm:h-9 text-xs sm:text-sm border border-gray-300 disabled:opacity-50 disabled:cursor-not-allowed"
                            />
                          </TableCell>
...
// Add this function above your BRVPrintView component
...
interface BRVPrintViewProps {
  entry: BRV;
  onClose: () => void;
  companyData: CompanyData | null;
}

const BRVPrintView: React.FC<BRVPrintViewProps> = ({ entry, onClose, companyData }) => {
  const componentRef = useRef<HTMLDivElement>(null);
  const [masterData, setMasterData] = useState<{ accounts: any[], branches: any[],
    // flocks: any[], 
     vouchers: any[], expenses: any[] }>({
    accounts: [], branches: [],
    // flocks: [], 
     vouchers: [], expenses: []
  });
  ...
  useEffect(() => {
    const fetchAllMasterData = async () => {
      try {
        const [accountsData, branchesData, vouchersData,
          // flocksData,
           expensesData] = await Promise.all([
          getAccounts(),
          getBranches(),
          getVouchers(),
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write('<html><head><title>Bank Receive Voucher</title>');
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-4xl max-h-[95vh] overflow-hidden flex flex-col">
        <CardHeader className="flex flex-row items-center justify-between no-print">
          <CardTitle>Print Bank Receipt Voucher</CardTitle>
            ...
            <div ref={componentRef}>
              ...
              <div className="voucher-header">
                ...
                <div className="title-section">
                  <h1 className="text-xl font-bold uppercase">Bank Receipt Voucher</h1>
                </div>
              </div>
