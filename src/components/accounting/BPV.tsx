...
const BPV: React.FC = () => {
  ...
  const [editingEntry, setEditingEntry] = useState<BPV | null>(null);
  const [showScrollToTop, setShowScrollToTop] = useState(false);
  const [selectedJournalEntry, setselectedJournalEntry] = useState<number[]>([]);
  const [printingEntry, setPrintingEntry] = useState<BPV | null>(null);
  const [statusFilter, setStatusFilter] = useState<string>('ALL');
  const [entries, setEntries] = useState<BPV[]>([]);
...
const filteredJE = entries.filter((entry) => {
  ...
  const handleEditEntry = (entry: BPV) => {
    setEditingEntry(entry);
    setShowForm(true);
  };

  const handleDeleteEntry = async (entry: BPV) => {
  ...
  const handlePrintEntry = (entry: BPV) => {
...
// In the BPV component, update handleSaveEntry to properly handle async
      ...
      <Card className="bg-white/80 backdrop-blur-sm shadow-lg">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Bank Payment Vouchers</CardTitle>
...
/>
                  ...
                  {/* <TableCell className="font-medium">{entry.expense_name || 'N/A'}
                    </TableCell>  */}
                  <TableCell>
...
const BPVForm: React.FC<BPVFormProps> = ({ onClose, onSave, entry }) => {
  ...
  const [creditAccountId, setCreditAccountId] = useState<number>(0); // Changed to creditAccountId
  const [vouchers, setVouchers] = useState<any[]>([]);
  const [voucher_id, setVoucherId] = useState<number>(7);
  ...
  const allowedAccountIds = [34245, 34246];
...
useEffect(() => {
  ...
  setEntryDate(formattedDate);
          ...
          // For editing existing entry, find credit account and debit lines
          if (entry.lines && Array.isArray(entry.lines) && entry.lines.length > 0) {
            // Find the credit line (where credit > 0)
            const creditLine = entry.lines.find(line => line.credit > 0);
            if (creditLine) {
              setCreditAccountId(creditLine.account_id);
            }
            
            // Filter debit lines (where debit > 0)
            const debitLines = entry.lines.filter(line => line.debit > 0);
            if (debitLines.length > 0) {
              setEntryLines(debitLines.map((line: any) => ({
                account_id: line.account_id || 0,
                item_id: line.item_id || 0,
                quantity: line.quantity || 0,
                rate: line.rate || 0,
                debit: line.debit || 0,
                credit: 0, // Set credit to 0 for debit lines
                description: line.description || ''
              })));
            } else {
              // If no debit lines, start with one empty line
  ...
  const updateLine = (index: number, field: keyof JournalLine, value: any) => {
    ...
    if (field === 'debit') {
      // Ensure debit amount is positive
      const debitValue = parseFloat(value) || 0;
      newLines[index] = { ...newLines[index], debit: debitValue };
    } else if (field === 'credit') {
      // Credit should always be 0 for debit lines
      newLines[index] = { ...newLines[index], credit: 0 };
    } else {
  ...
  // Calculate total debit (sum of all debit lines)
  const totalDebit = entryLines.reduce((sum, line) => sum + (line.debit || 0), 0);
  
  // Total credit equals total debit (automatically assigned to credit account)
  const totalCredit = totalDebit;
  
  // Validation: Check if credit account is selected and there's at least one debit line with amount
  const isValid = creditAccountId > 0 && totalDebit > 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Filter debit lines with valid accounts and amounts
      const debitLines = entryLines.filter(line => line.account_id > 0 && line.debit > 0);

      if (debitLines.length === 0) {
        throw new Error("At least one debit entry is required");
      }
      ...
      const payload = {
        ...
        description,
        lines: [...debitLines, creditLine] // Debit lines first, then credit line
      };
      ...
      <Card className="w-full max-w-4xl max-h-[98vh] overflow-hidden rounded-3xl border-0 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 shadow-2xl flex flex-col">
        ...
        <CardHeader className="flex flex-row items-center justify-between bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 text-white rounded-t-3xl px-4 sm:px-6 py-3 sm:py-4 flex-shrink-0">
          <CardTitle className="text-lg sm:text-xl font-bold tracking-wide text-white">
            {entry ? 'Edit BPV Entry' : 'Bank Payment Voucher'}
          </CardTitle>
          <Button
            variant="ghost"
            size="sm"
                      ...
                      {entryLines.map((line, index) => (
                        <TableRow key={index} className="hover:bg-purple-50">
                          <TableCell className="py-1 px-1 sm:px-2">
                            <Popover>
                              <PopoverTrigger asChild>
                                <Button className="w-full justify-between bg-indigo-50 border border-purple-500 text-black h-8 sm:h-9 text-xs sm:text-sm px-2">
                                  {line.account_id
...
// Add this function above your BPVPrintView component
...
interface BPVPrintViewProps {
  entry: BPV;
  onClose: () => void;
  companyData: CompanyData | null;
}

const BPVPrintView: React.FC<BPVPrintViewProps> = ({ entry, onClose, companyData }) => {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write('<html><head><title>Bank Payment Voucher</title>');
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-4xl max-h-[95vh] overflow-hidden flex flex-col">
        <CardHeader className="flex flex-row items-center justify-between no-print">
          <CardTitle>Print Bank Payment Voucher</CardTitle>
            ...
            <div ref={componentRef}>
              ...
              <div className="voucher-header">
                ...
                <div className="title-section">
                  <h1 className="font-bold uppercase">Bank Payment Voucher</h1>
                </div>
              </div>
                  ...
                  {entry.lines.map((line, index) => {
                    ...
                    return (
                      <TableRow key={index}>
                        ...
                        <TableCell className="border-black text-left">{formatNumber(line.debit)}</TableCell>
