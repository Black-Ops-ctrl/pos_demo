...
const CPV: React.FC = () => {
  ...
  const [editingEntry, setEditingEntry] = useState<CPV | null>(null);
  const [showScrollToTop, setShowScrollToTop] = useState(false);
  const [selectedJournalEntry, setselectedJournalEntry] = useState<number[]>([]);
  const [printingEntry, setPrintingEntry] = useState<CPV | null>(null);
  const [statusFilter, setStatusFilter] = useState<string>('ALL');
  const [entries, setEntries] = useState<CPV[]>([]);
...
const filteredJE = entries.filter((entry) => {
  ...
  const handleEditEntry = (entry: CPV) => {
    setEditingEntry(entry);
    setShowForm(true);
  };

  const handleDeleteEntry = async (entry: CPV) => {
  ...
  const handlePrintEntry = (entry: CPV) => {
...
const handleSaveEntry = async (payload: {
  ...
  description: string;
  lines: JournalLine[];
}): Promise<boolean> => { // Add return type Promise<boolean>
  try {
    ...
    throw err; // Re-throw the error so CPVForm can catch it
  }
};
      ...
      <Card className="bg-white/80 backdrop-blur-sm shadow-lg">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Cash Payment Vouchers</CardTitle>
...
const CPVForm: React.FC<CPVFormProps> = ({ onClose, onSave, entry }) => {
  ...
  const [creditAccountId, setCreditAccountId] = useState<number>(34249); // Changed to creditAccountId
  const [vouchers, setVouchers] = useState<any[]>([]);
  const [voucher_id, setVoucherId] = useState<number>(8);
...
// Add this function above your CPVPrintView component
...
interface CPVPrintViewProps {
  entry: CPV;
  onClose: () => void;
  companyData: CompanyData | null;
}

const CPVPrintView: React.FC<CPVPrintViewProps> = ({ entry, onClose, companyData }) => {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write('<html><head><title>Cash Payment Voucher</title>');
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-4xl max-h-[95vh] overflow-hidden flex flex-col">
        <CardHeader className="flex flex-row items-center justify-between no-print">
          <CardTitle>Print Cash Payment Voucher</CardTitle>
            ...
            <div ref={componentRef}>
              ...
              <div className="voucher-header">
                ...
                <div className="title-section">
                  <h1 className="text-xl font-bold uppercase">Cash Payment Voucher</h1>
                </div>
              </div>
