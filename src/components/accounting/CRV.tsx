...
const CRVForm: React.FC<CRVFormProps> = ({ onClose, onSave, entry }) => {
  ...
  const [debitAccountId, setDebitAccountId] = useState<number>(0); // Renamed from account_id to debitAccountId
          ...
          if (entry.lines && Array.isArray(entry.lines) && entry.lines.length > 0) {
            ...
            if (creditLines.length > 0) {
              setEntryLines(creditLines.map((line: JournalLine) => ({
                ...
                rate: line.rate || 0,
                debit: 0, // Set debit to 0 for credit lines
                credit: line.credit || 0,
                description: line.description || ''
              })));
            } else {
              // If no credit lines, start with one empty line
  ...
  const updateLine = (index: number, field: keyof JournalLine, value: any) => {
    ...
    if (field === 'credit') {
      ...
      // Debit should always be 0 for credit lines
  ...
  // Calculate total credit (sum of all credit lines)
  const totalCredit = entryLines.reduce((sum, line) => sum + (line.credit || 0), 0);
  
  // Total debit equals total credit (automatically assigned to debit account)
  const totalDebit = totalCredit;
  
  // Validation: Check if debit account is selected and there's at least one credit line with amount
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Filter credit lines with valid accounts and amounts
      ...
      const payload = {
        ...
        description,
        lines: [debitLine, ...creditLines] // Debit line first, then credit lines
      };
  ...
  <Card className="w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-3xl border-0 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 shadow-2xl">
    ...
    <CardHeader className="flex flex-row items-center justify-between bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 text-white rounded-t-3xl mb-4">
      <CardTitle className="text-xl text-white font-bold tracking-wide drop-shadow">
        {entry ? 'Edit CRV Entry' : 'New CRV Entry'}
      </CardTitle>
      <Button
        variant="ghost"
        size="sm"
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="rounded-2xl p-6 bg-gradient-to-br from-white to-indigo-50 border border-indigo-200 shadow-md">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <Popover open={branchOpen} onOpenChange={setBranchOpen}>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    role="combobox"
                    className="w-full justify-between mt-1 bg-gradient-to-r from-indigo-100 to-purple-100 border-indigo-300"
                  >
                    {branch_id
            ...
            {/* DEBIT ACCOUNT */}
            <div>
              <Label className="text-indigo-700">Debit Account</Label>
              <Popover open={accountOpen} onOpenChange={setAccountOpen}>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    role="combobox"
                    className="w-full justify-between mt-1 bg-gradient-to-r from-pink-100 to-indigo-100 border-indigo-300"
                  >
                    {debitAccountId
                      ? `${accounts.find(a => a.account_id === debitAccountId)?.account_name}
                         (${accounts.find(a => a.account_id === debitAccountId)?.account_code})`
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gradient-to-r from-purple-500 to-indigo-500">
                  <TableHead className="text-white">Credit Account</TableHead>
