...
const [endDate, setEndDate] = useState<string>(getTodayDate);
  const [editingEntry, setEditingEntry] = useState<CRV | null>(null);
  const [showScrollToTop, setShowScrollToTop] = useState(false);
  const [selectedJournalEntry, setselectedJournalEntry] = useState<number[]>([]);
  const [printingEntry, setPrintingEntry] = useState<CRV | null>(null);
  const [statusFilter, setStatusFilter] = useState<string>('ALL');
  const [entries, setEntries] = useState<CRV[]>([]);
...
// useEffect(() => {
//   // Load entries only when both dates are selected
//   if (startDate && endDate) {
//     loadJournalEntries();
//   } else {
//     // Clear entries when dates are not selected
//     setEntries([]);
//   }
// }, [startDate, endDate]); //
...
const filteredJE = entries.filter((entry) => {
  ...
  const handleEditEntry = (entry: CRV) => {
    setEditingEntry(entry);
    setShowForm(true);
  };

  const handleDeleteEntry = async (entry: CRV) => {
  ...
  const handlePrintEntry = (entry: CRV) => {
...
// In the CRV component, update handleSaveEntry to properly handle async
const handleSaveEntry = async (payload: {
  ...
  try {
    ...
    // Optional: Validate journal lines balance (debit = credit)
    ...
    if (totalDebit !== totalCredit) {
      ...
      throw new Error("Debit/Credit not balanced");
    }
    ...
    if (!err.message.includes("required") && !err.message.includes("balanced")) {
      toast({
        title: "Error",
...
<CardHeader>
  <div className="flex items-center justify-between">
    <CardTitle>Cash Receipt Vouchers</CardTitle>
    ...
    <div className="flex justify-end gap-2 mt-2">
      {selectedJournalEntry.length > 0 && (
        <>
          {/* Show UnApprove button only if selected entries include APPROVED ones */}
          ...
          {/* Show Approve button only if selected entries include CREATED ones */}
          {selectedJournalEntry.some(id =>
...
const CRVForm: React.FC<CRVFormProps> = ({ onClose, onSave, entry }) => {
  ...
  const [voucher_id, setVoucherId] = useState<number>(4);
  ...
  const branchPlaceholder = module_id === 3 ? "Select Branch" : "Select Farm";

  useEffect(() => {
    const fetchData = async () => {
      try {
        const [accountsData, branchData, vouchersData] =
          await Promise.all([
            getAccounts(),
            getBranches(),
            getVouchers(),
        ...
        if (entry) {
          ...
          if (entry.lines && Array.isArray(entry.lines) && entry.lines.length > 0) {
            ...
            if (debitLine) {
              setDebitAccountId(debitLine.account_id);
            }
            
            const creditLines = entry.lines.filter(line => line.credit > 0);
            if (creditLines.length > 0) {
              setEntryLines(creditLines.map((line: JournalLine) => ({
              ...
              setEntryLines([{ account_id: 0, item_id: 0, quantity: 0, rate: 0, debit: 0, credit: 0, description: '' }]);
            }
          } else {
            setEntryLines([{ account_id: 0, item_id: 0, quantity: 0, rate: 0, debit: 0, credit: 0, description: '' }]);
          }
  ...
  const updateLine = (index: number, field: keyof JournalLine, value: any) => {
    ...
    if (field === 'credit') {
      ...
      newLines[index] = { ...newLines[index], credit: creditValue };
    } else if (field === 'debit') {
      newLines[index] = { ...newLines[index], debit: 0 };
    } else {
  ...
  const isValid = debitAccountId > 0 && totalCredit > 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const creditLines = entryLines.filter(line => line.account_id > 0 && line.credit > 0);

      if (creditLines.length === 0) {
        throw new Error("At least one credit entry is required");
      }
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 p-2 sm:p-4">
      <Card className="w-full max-w-4xl max-h-[98vh] overflow-hidden rounded-3xl border-0 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 shadow-2xl flex flex-col">
        ...
        <CardHeader className="flex flex-row items-center justify-between bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 text-white rounded-t-3xl px-4 sm:px-6 py-3 sm:py-4 flex-shrink-0">
          <CardTitle className="text-lg sm:text-xl text-white font-bold tracking-wide drop-shadow">
            {entry ? 'Edit CRV Entry' : 'Cash Receipt Voucher'}
          </CardTitle>
          <Button
            variant="ghost"
            size="sm"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    required
                    className="mt-1 bg-white border-indigo-300 focus:ring-2 focus:ring-pink-400 focus:border-pink-400 h-9 sm:h-10 text-sm"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    required
                    className="mt-1 h-9 sm:h-10 w-full rounded-md border border-indigo-300 bg-white px-3 text-sm
                               focus:ring-2 focus:ring-purple-400 focus:border-purple-400"
                  >
                    <option value="" disabled>Select voucher</option>
                    {vouchers
                      .filter(v => v.voucher_name === "CRV")
                      .map(v => (
                  ...
                  <Popover open={branchOpen} onOpenChange={setBranchOpen}>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        role="combobox"
                        className="w-full justify-between mt-1 bg-gradient-to-r from-indigo-100 to-purple-100 border-indigo-300 h-9 sm:h-10 text-sm"
                      >
                        {branch_id
                          ? branches.find(b => b.branch_id === branch_id)?.branch_name
                          : 'Select Branch'}
                          ...
                          {branches.map(b => (
                            ...
                            >
                              <Check className={cn(
                                ...
                                branch_id === b.branch_id ? "opacity-100" : "opacity-0"
                              )} />
                              {b.branch_name}
                            </CommandItem>
                          ))}
                ...
                <div className="sm:col-span-2 lg:col-span-1">
                  <Label className="text-indigo-700 text-xs sm:text-sm">Debit Account</Label>
                  <Popover open={accountOpen} onOpenChange={setAccountOpen}>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        role="combobox"
                        className="w-full justify-between mt-1 bg-gradient-to-r from-pink-100 to-indigo-100 border-indigo-300 h-9 sm:h-10 text-sm"
                      >
                        {debitAccountId
                          ? `${accounts.find(a => a.account_id === debitAccountId)?.account_name}`
                        ...
                        <CommandGroup>
                          {accounts.map(a => (
                            <CommandItem
                              key={a.account_id}
                              onSelect={() => {
                                setDebitAccountId(a.account_id);
                            ...
                            >
                              <Check className={cn(
                                "mr-2 h-3 w-3 sm:h-4 sm:w-4 text-green-500",
                                debitAccountId === a.account_id ? "opacity-100" : "opacity-0"
                              )} />
                ...
                <div className="sm:col-span-2 lg:col-span-2">
                  ...
                  <Input
                    ...
                    required
                    className="mt-1 bg-white border-indigo-300 focus:ring-2 focus:ring-purple-400 h-9 sm:h-10 text-sm"
                  />
                </div>
                  ...
                  <Table>
                    <TableHeader className="sticky top-0 z-10">
                      <TableRow className="bg-gradient-to-r from-purple-500 to-indigo-500">
                        <TableHead className="text-white py-2 text-xs sm:text-sm whitespace-nowrap">Credit Account</TableHead>
                        <TableHead className="text-white py-2 text-xs sm:text-sm">Description</TableHead>
                        <TableHead className="text-white py-2 text-xs sm:text-sm">Amount</TableHead>
                      ...
                      {entryLines.map((line, index) => (
                        <TableRow key={index} className="hover:bg-purple-50">
                          <TableCell className="py-1 px-1 sm:px-2">
                            <Popover>
                              <PopoverTrigger asChild>
                                <Button className="w-full justify-between bg-indigo-50 border-indigo-300 text-black h-8 sm:h-9 text-xs sm:text-sm px-2 border border-purple-500">
                                  {line.account_id
                        ...
                        <Input
                          ...
                          className="bg-indigo-50 h-8 sm:h-9 text-xs sm:text-sm border border-purple-500 disabled:opacity-50 disabled:cursor-not-allowed"
                        />
                      </TableCell>
              ...
              <div className="mt-3 p-2 sm:p-3 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 text-white text-center shadow-md">
                ...
                {showBalanceError && (
                  <div className="mt-1 text-xs sm:text-sm text-yellow-200">
                    Please select a debit account and enter credit amount.
                  </div>
                )}
              </div>
            </div>
...
// Add this function above your CRVPrintView component
...
interface CRVPrintViewProps {
  entry: CRV;
  onClose: () => void;
  companyData: CompanyData | null; // Add this line
}

const CRVPrintView: React.FC<CRVPrintViewProps> = ({ entry, onClose, companyData }) => {
    const componentRef = useRef<HTMLDivElement>(null);
  const [masterData, setMasterData] = useState<{ accounts: any[], branches: any[], flocks: any[], vouchers: any[], expenses: any[] }>({
    accounts: [], branches: [], flocks: [], vouchers: [], expenses: []
  });

  useEffect(() => {
    const fetchAllMasterData = async () => {
      try {
        const [accountsData, branchesData, vouchersData, flocksData, expensesData] = await Promise.all([
          getAccounts(),
          getBranches(),
          getVouchers(),
          getFlockByBranch(entry.branch_id),
          getExpense(), // Fetch expenses for print view
        ]);

        // Filter only approved branches for print view as well
 ...
 // Format number with commas and remove decimal zeros
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write('<html><head><title>Cash Receipt Voucher</title>');
     ...
     <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
       <Card className="w-full max-w-4xl max-h-[95vh] overflow-hidden flex flex-col">
         <CardHeader className="flex flex-row items-center justify-between no-print">
           <CardTitle>Print Cash Receipt Voucher</CardTitle>
             ...
             <div ref={componentRef}>
               ...
               <div className="voucher-header">
                 <div className="logo-section">
                   <img
                     src={getLogoSource()} // Change this line
                     alt={companyData?.company_name || "Company Logo"} // Change this line
                     className="logo"
                   />
                 </div>
                 <div className="title-section">
                   <h1 className="text-xl font-bold uppercase">Cash Receipt Voucher</h1>
                 </div>
                 {/* <div className="details-section">
                   <div className="text-sm">
                     <div><strong>Reference: </strong> {entry.reference_number || 'N/A'}</div>
                   </div>
                 </div> */}
               </div>
               ...
               <div className="master-data-box">
                 <div className="master-data-grid">
                   <div className="master-data-col">
                     ...
                     {/* <div className="master-data-item">
                       <span className="master-data-label">Expense:</span>
                       <span>{getMasterName(masterData.expenses, entry.expense_id, 'expense_id', 'expense_name')}</span>
                     </div> */}
                   </div>
               ...
               {/* <h3 className="text-lg font-bold my-4 text-center">JOURNAL DETAILS</h3> */}
