...
import { getBirdsVehicleLedger } from "@/api/BirdsvehicleApi";
...
const Birdsvehicle: React.FC = () => {
  ...
  const loadReport = async () => {
    ...
    try {
      const data = await getBirdsVehicleLedger(start_date, end_date);
...
const formatDate = (dateString: string) => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Birds Vehicle Ledger Report</CardTitle>
      </CardHeader>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50">
                      ...
                      <TableHead className="font-semibold">Purchase Invoice No</TableHead>
                      <TableHead className="font-semibold">Sale Invoice No</TableHead>
                  ...
                  <TableBody>
                    {filteredData.map((entry, index) => (
                      <TableRow key={`${entry.model_sequence}-${index}`} className="hover:bg-gray-50">
                        ...
                        <TableCell className="font-medium">{entry.purchase_invoice_no}</TableCell>
                          <TableCell className="font-medium">{entry.sale_invoice_no}</TableCell>
